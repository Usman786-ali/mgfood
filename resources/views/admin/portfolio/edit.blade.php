...
@section('content')
    <div class="form-card">
        <h2 style="margin-bottom: 30px; font-size: 24px; font-weight: 700;">Edit Portfolio Item</h2>
        ...
        <form method="POST" action="{{ route('admin.portfolio.update', $portfolio) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select id="category" name="category" required>
                    ...
                    <option value="Wedding" {{ old('category', $portfolio->category) == 'Wedding' ? 'selected' : '' }}>Wedding
                    </option>
                    <option value="Corporate" {{ old('category', $portfolio->category) == 'Corporate' ? 'selected' : '' }}>
                        Corporate</option>
                    <option value="Birthday" {{ old('category', $portfolio->category) == 'Birthday' ? 'selected' : '' }}>
                        Birthday</option>
                    <option value="Social" {{ old('category', $portfolio->category) == 'Social' ? 'selected' : '' }}>Social
                        Event</option>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                @if($portfolio->image)
                    <div style="margin-bottom: 10px;">
                        <img src="{{ asset('storage/' . $portfolio->image) }}" alt="Current"
            ...
            <div class="form-group">
                ...
                <input type="number" id="order" name="order" value="{{ old('order', $portfolio->order) }}" min="0">
            </div>
